...
class ShortTestnetValidator:
    def __init__(self):
        ...
        self.test_duration = 300  # 5 minutes test instead of 1 hour
    ...
    def start_node(self, node):
        """Start a Udaya node using the compiled binary"""
        # Use the compiled binary directly to avoid cargo run overhead
        binary = os.path.join("target", "release", "Udayad.exe")
        if not os.path.exists(binary):
            binary = os.path.join("target", "release", "Udayad")
        if not os.path.exists(binary):
            self.log(f"Binary not found at {binary}. Building first...", "WARNING")
            returncode, stdout, stderr = self.run_command("cargo build --release")
            if returncode != 0:
                self.log(f"Build failed: {stderr}", "ERROR")
                return False

        cmd = [binary, "-c", node["config"], "start"]
        self.log(f"Starting node {node['id']} with command: {cmd}")

        try:
            # Start the process with output redirected to a log file
            log_file = f"temp_testnet_data/node{node['id']}/node{node['id']}.log"
            os.makedirs(os.path.dirname(log_file), exist_ok=True)
            log_handle = open(log_file, 'w')
            # Use shell=False to directly get the Udayad process PID
            ...
            # Wait for the node to initialize and check if it's still running
            time.sleep(8)
            ...
            if process.poll() is None:
                ...
                if healthy:
                    self.log(f"Node {node['id']} is running and RPC is responding")
                    return True
                else:
                    self.log(f"Node {node['id']} process running but RPC not responding: {status}", "WARNING")
                    # Give it more time
                    time.sleep(5)
                    healthy, status = self.check_node_health(node)
                    if healthy:
                        self.log(f"Node {node['id']} RPC now responding")
                        return True
                    self.log(f"Node {node['id']} RPC still not responding: {status}", "ERROR")
                    return False
            else:
                self.log(f"Node {node['id']} failed to start (exit code {process.returncode})", "ERROR")
                # Read the log for error details
                try:
                    with open(log_file, 'r') as f:
                        log_content = f.read()
                    self.log(f"Node {node['id']} log: {log_content[-500:]}", "ERROR")
                except Exception:
                    pass
                return False
    ...
    def generate_test_transactions(self):
        ...
        # Initialize transaction stats if not present
    ...
    def generate_final_report(self):
        ...
        report_file = "SHORT_TESTNET_VALIDATION_REPORT.json"
    ...
    def _write_human_readable_summary(self, report):
        ...
        summary_file = "SHORT_TESTNET_VALIDATION_SUMMARY.md"
        data = report['testnet_validation_report']

        with open(summary_file, 'w', encoding='utf-8') as f:
            f.write("# Udaya Blockchain Short Testnet Validation Report\n\n")
    ...
    def run_validation(self):
        ...
        self.log("Starting Udaya Blockchain Short Testnet Validation")
        self.start_time = datetime.now()
        self.running = True

        try:
            # Step 0: Clean up any stale test data and lock files
            self.log("Cleaning up stale test data...")
            for node in self.nodes:
                data_dir = f"temp_testnet_data/node{node['id']}"
                if os.path.exists(data_dir):
                    import shutil
                    shutil.rmtree(data_dir, ignore_errors=True)
            self.log("Stale test data cleaned")
            ...
            fix_ok = self.detect_and_fix_issues()
            if not fix_ok:
                self.log("Failed to fix repository-resolvable issues", "ERROR")
                return False
            ...
            self.log("Short testnet validation completed")
            return True
...
if __name__ == "__main__":
    validator = ShortTestnetValidator()

    try:
        success = validator.run_validation()

        if success:
            validator.log("✅ Short testnet validation completed successfully")
            sys.exit(0)
        else:
            validator.log("❌ Short testnet validation completed with issues", "ERROR")
            sys.exit(1)
